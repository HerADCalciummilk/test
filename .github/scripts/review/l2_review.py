...
def collect_package_context(repo: Path, package: Path) -> str:
    ...
    if used == 0:
        chunks.append("（包内未采集到可读的 docs/src/cli 文本）\n")
    return "\n".join(chunks)
...
def main() -> int:
    ...
    package_strs = [p.as_posix() for p in packages]
    ...
    if not packages:
        ...
        print(f"L2 跳过：无算法包 report={out}")
        return 0

    if args.dry_run:
        result = normalize_model_result(dry_run_result(package_strs))
        report = build_report(
            package_strs,
            model="dry-run",
            skipped=False,
        ...
        out.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"L2 dry-run 完成：packages={package_strs} report={out}")
        return 0
