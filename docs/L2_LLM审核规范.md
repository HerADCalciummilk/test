...
规范与风险标准一致；按**实际出现的材料**决定专项（有则查、无则不臆造）。

**通用（有依据再写）：** 逻辑是否合理、与可见 docs 是否一致、硬编码/隐蔽 I/O/不安全执行等、其它入库前应知晓的问题。
...
| 含可读 cli | 须判断是否调用插件业务；仅占位则写入 findings；无 cli 则不查、不臆造缺 cli |
| 结构/六树 | 不做 L1 已负责的结构门禁推断 |

正式算法材料中的 cli 通常在仓库根 `cli/<kind>/<pkg>/`。
